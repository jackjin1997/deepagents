...
def _log_efficiency(
    ...
    for index, assertion in enumerate(scorer._expectations, start=1):
        feedback_key = {
            AgentSteps: "efficiency_agent_steps",
            ToolCallRequests: "efficiency_tool_call_requests",
            MaxToolCallRequests: "efficiency_max_tool_call_requests",
            ToolCall: "efficiency_tool_call",
        }.get(type(assertion), "efficiency_assertion")
        feedback_key = f"{feedback_key}_{index}"
