...
_FILE_MUTATION_TOOLS: Final[frozenset[str]] = frozenset({"write_file", "edit_file", "delete"})
"""Filesystem tools that must run sequentially when they target one path."""
...
def _parallel_file_mutation_error(request: ToolCallRequest) -> ToolMessage | None:
    """Reject a file mutation preceded by another same-path mutation in its batch."""
    ...
    if not isinstance(file_path, str) or not isinstance(messages, Sequence):
        return None
    ai_message = next((message for message in reversed(messages) if isinstance(message, AIMessage)), None)
    if ai_message is None:
        return None

    for previous_call in ai_message.tool_calls:
        if previous_call["id"] == tool_call["id"]:
            return None
        if previous_call["name"] in _FILE_MUTATION_TOOLS and previous_call["args"].get("file_path") == file_path:
            content = f"Error: parallel filesystem mutations target '{file_path}'. Run mutations for the same path sequentially."
            return _tool_error(tool_call["name"], tool_call["id"], content)
    return None
